...
def get_extracted_data(request):
    if env == "prod":
        ...
        for entry in all_entries:
            item = FileTextSerializer(
                FileText.objects.filter(
                    file_id=entry['file_id']), many=True)
        ...
        for _file in resp_json:
            item = FileSerializer(
                ...
                many=True)
            _file["file_name"] = json.loads(json.dumps(item.data))[
                0]["file_name"]
        if modal_name not in list(modal_classifications.keys()):
            modal_details = pd.DataFrame.from_records(
                TrainedModel.objects.filter(
                    model_name=modal_name).values())
            if not modal_details.empty:
                modal_id = int(modal_details.iloc[-1]['model_id'])
            get_details = pd.DataFrame.from_records(
                FilesForTrainingModel.objects.filter(
                    model_id=modal_id).values())
            modal_classifications_list = list(
                set(get_details['folder_name'].tolist()))
    ...
    if env == "dev":
        ...
        modal_classifications_list = modal_classifications[modal_name]
        classification_count = dict.fromkeys(modal_classifications_list, 0)
        resp_json = [{'class': 'Additional Monitoring Activity [Site Handover Form]',
                      'confidence': 0.22228756546974182,
                      'error': 'Success',
                      'model': 'Clinical Document Classification',
                      'file_name': 'Document1.pdf',
                      'page_text': 'something'},
                     {'class': 'Summary of Clinical Efficacy',
                      'confidence': 0.8513577230114553,
                      'error': 'Success',
                      'model': 'CORONA',
                      'file_name': 'Document3.pdf',
                      'page_text': 'text of the file 2'}]
        return Response(
            data={
                'data': resp_json,
                'summary': classification_count})
    ...
    if request.method == "POST":
        ...
        for myfile in request.FILES.getlist('media'):
            ...
            file_serializer = FileSerializer(
                data={"file_path": file_url, 'file_name': filename})
            if (file_serializer.is_valid(raise_exception=True)):
                ...
                ft_serializer = FileTextSerializer(
                    data={'file_id': file_obj.file_id, 'file_text': file_text})
...
def upload_files_for_training_model(request):
    if request.method == "GET":
        ...
        return render(
            request,
            'ui_upload_document_types.html',
            context=context)
    if request.method == "POST":
        get_details = pd.DataFrame.from_records(
            FilesForTrainingModel.objects.filter(
                is_extracted='False').values())
        if len(get_details) > 0:
            return render(
                request, 'model_progress.html', context={
                    'msg': 'Warning: Already another modal file extraction is in progress. Server cannot take any more requests'})
        ...
        return render(request, 'model_progress.html', context={
                      'msg': 'Model Training is in Progress'})
    return Response(status=status.HTTP_400_BAD_REQUEST)
